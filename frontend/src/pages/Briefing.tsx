      ...
      <main className="briefing-main">
        ...
        <article className="briefing-document">
          ...
          <section className="briefing-section">
             <div className="briefing-section-title"><h2>Executive Assessment (Plain English)</h2></div>
          ...
          <section className="briefing-section"><div className="briefing-section-title"><h2>Attack Progression Map</h2></div><div className="briefing-stages">{data.stages.map((stage) => <div className={`briefing-stage briefing-stage-${stage.status}`} key={stage.name} style={{ opacity: stage.status === "pending" ? 0.4 : 1, border: stage.status === "current" ? "1px solid #00e5ff" : undefined }}><span>{stage.status === "current" ? "HAPPENING NOW" : stage.status === "complete" ? "ALREADY OCCURRED" : "EXPECTED LATER"}</span><strong style={{ color: stage.status === "current" ? "#00e5ff" : undefined }}>{stage.name}</strong><small>{stage.tactic}</small></div>)}</div></section>
          <section className="briefing-two-col">
            <div className="briefing-section" style={{ width: "100%" }}><div className="briefing-section-title"><h2>What triggered this warning?</h2><small>The specific network behaviors the AI flagged</small></div><div className="briefing-evidence">{data.attributions.map((item, index) => <div key={item.feature} style={{ padding: "12px", borderBottom: "1px solid #2b3b31" }}><span>0{index + 1}</span><strong style={{ fontSize: "14px", display: "block", marginBottom: "4px" }}>{item.feature}</strong><em style={{ color: "#ff4d30", display: "block", marginBottom: "4px" }}>Impact: {Math.round(item.weight * 100)}%</em><small style={{ color: "#9aaba3" }}>{item.value} · {item.explanation}</small></div>)}</div></div>
          </section>
          {data.notes.length > 0 && <section className="briefing-section" data-testid="briefing-notes-section"><div className="briefing-section-title"><h2>Analyst replay notes</h2><small>{data.notes.length} saved observation{data.notes.length === 1 ? "" : "s"}</small></div><div className="briefing-notes">{data.notes.map((note) => <div className="briefing-note" key={note.id}><div><span>{note.window_label}</span><b className={`note-category note-category-${note.category.toLowerCase()}`}>{note.category}</b></div><p>{note.text}</p><small>{note.updated_at ? `Updated ${new Date(note.updated_at).toLocaleString()}` : new Date(note.created_at).toLocaleString()}</small></div>)}</div></section>}
